...
int parallel_solve(Sudoku *sudoku_head, int max_branched_sudokus, double *end_times)
{
    ...
    while ((branched_sudokus < max_branched_sudokus) && (has_next_cell))
    {
        ...
        Cell unfilled_cell;
        has_next_cell = get_unfilled_cell(curr_sudoku, &unfilled_cell);
        if (has_next_cell)
        {
            branched_sudokus--;
            int row = unfilled_cell.row, col = unfilled_cell.col;
            // printf("Unfilled Row %d Col %d\n", row, col);
            // show_grid(&curr_sudoku->grid); 
            // show_heap(&curr_sudoku->heap);
            int safe_value = 0;
            for (int v = 1; v <= N; v++)
            {
